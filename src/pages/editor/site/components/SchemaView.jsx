...
const SechmaItem = ({ name, value, properties }) => {
  ...
  if (properties.type === 'text') {
    component = (
      <TextArea value={value} onChange={() => {}} autoSize={{ minRows: 3, maxRows: 5 }} />
    );
  }
  ...
  return (
    <div className={styles.sechmaPropItem}>
      <div className={styles.sechmaPropTitle}>
        ...
        {properties.meta && properties.meta.description && (
...
const SechmaList = ({ model }) => (
  <div className={styles.sechmaItemContainer}>
    ...
    <Collapse
      accordion
      expandIcon={({ isActive }) => (
        <span className={styles.sechmaCollapseIcon} rotate={isActive ? 90 : 0} />
      )}
    >
            ...
            ([propIndex, propItem]) =>
              propIndex !== 'key' && (
                <SechmaItem
                  ...
                  name={propIndex}
                  value={propItem}
                  properties={model.json.properties[propIndex]}
                />
              ),
          )}
        </Panel>
  ...
  if (model.papilioPackage) {
    Object.entries(model.papilioPackage.schemas).forEach(([key, sechma]) => {
      ...
      if (sechma.json.type === 'array') {
        components.push(<SechmaList model={sechma} key={sechma.key} />);
      }
      ...
      if (sechma.json.type === 'object') {
        components.push(
          <div className={styles.sechmaItemContainer} key={sechma.key}>
            ...
            {Object.entries(sechma.data).map(
              ([propIndex, propItem]) =>
                propIndex !== 'key' && (
                  <SechmaItem
                    key={propIndex}
                    name={propIndex}
                    value={propItem}
                    properties={sechma.json.properties[propIndex]}
                  />
                ),
            )}
          </div>,
